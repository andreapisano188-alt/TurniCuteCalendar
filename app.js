/**
 * TurniCute Calendar - Application Logic (Orari Aggiornati)
 */

// Presets dei turni aggiornati
const SHIFT_PRESETS = {
    ...
    if (shift) {
      ...
      innerHTML += `
        <div class="shift-badge ${preset.cssClass}">
          ...
          ${shift.type === 'ferie' ? `<span class="shift-hours-text">Ferie (6h)</span>` : (shift.type !== 'riposo' ? `<span class="shift-hours-text">${shift.start}-${shift.end} (${hoursNet}h)</span>` : '')}
        </div>
      `;
    }
...
function handleQuickFillSubmit(e) {
  ...
  if (patternSelect === '2M-2P-2N-2R') {
    patternArray = ['mattina', 'mattina', 'pomeriggio', 'pomeriggio', 'notte', 'notte', 'riposo', 'riposo'];
  } else if (patternSelect === '1M-1P-1N-1R') {
    patternArray = ['mattina', 'pomeriggio', 'notte', 'riposo'];
    ...
    const map = { 'M': 'mattina', 'P': 'pomeriggio', 'N': 'notte', 'F': 'ferie', 'R': 'riposo' };
    patternArray = raw.split(',').map(s => map[s.trim()]).filter(Boolean);
  }

  if (patternArray.length === 0) {
    alert("Sequenza non valida! Usa M, P, N, F, R separati da virgola.");
    return;
  }
...
function updateModalStats() {
  const counts = { mattina: 0, pomeriggio: 0, notte: 0, ferie: 0, riposo: 0, extra: 0 };
  const hours = { mattina: 0, pomeriggio: 0, notte: 0, ferie: 0, riposo: 0, extra: 0 };
  ...
  const totalShifts = (counts.mattina + counts.pomeriggio + counts.notte + counts.ferie + counts.riposo) || 1;
...
function calculateNetHoursForShift(shift) {
  ...
  if (shift.type === 'riposo') return 0.0;
